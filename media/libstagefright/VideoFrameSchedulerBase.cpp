...
nsecs_t VideoFrameSchedulerBase::schedule(nsecs_t renderTime) {
    ...
    if (videoPeriod > 0) {
        ...
        for (size_t i = 1; i <= N; i++) {
            offset +=
// QTI_BEGIN: 2023-09-12: Video: media: libstagefright: Change to avoid ubsan_sub_overflow error
                (renderTime + mTimeCorrection + videoPeriod * (nsecs_t)i - mVsyncTime)
                    % mVsyncPeriod;
            edgeRemainder += (videoPeriod * (nsecs_t)i) % mVsyncPeriod;
// QTI_END: 2023-09-12: Video: media: libstagefright: Change to avoid ubsan_sub_overflow error
        }
